...
<div class="w-full" x-data="{ 
    mainPreview: '<?php echo e($product->image_path ? asset('storage/' . $product->image_path) : ''); ?>',
    gallery: <?php echo e(json_encode($product->gallery ?? [])); ?>,
    removedGallery: [],
    newGalleryPreviews: [],
    
    removeExisting(path) {
        this.removedGallery.push(path);
        this.gallery = this.gallery.filter(img => img !== path);
    },
    ...
    <div class="mb-6">
        <a href="<?php echo e(route('admin.products.index')); ?>" class="text-slate-400 hover:text-white flex items-center space-x-2 transition-all">
                ...
                <div class="space-y-6">
                    <div>
                        ...
                        <input type="text" name="name" id="name" value="<?php echo e(old('name', $product->name)); ?>" required
                            class="w-full bg-slate-900 border border-slate-700 rounded-xl px-5 py-4 text-white focus:outline-none focus:border-red-500 focus:ring-1 focus:ring-red-500 transition-all <?php $__errorArgs = ['name'];
...
unset($__errorArgs, $__bag); ?>
                    ...
                    <div>
                        ...
                        <input type="text" name="sku" id="sku" value="<?php echo e(old('sku', $product->sku)); ?>" required
                            class="w-full bg-slate-900 border border-slate-700 rounded-xl px-5 py-4 text-white focus:outline-none focus:border-red-500 focus:ring-1 focus:ring-red-500 transition-all <?php $__errorArgs = ['sku'];
$__bag = $errors->getBag($__errorArgs[1] ?? 'default');
if ($__bag->has($__errorArgs[0])) :
if (isset($message)) { $__messageOriginal = $message; }
$message = $__bag->first($__errorArgs[0]); ?> border-red-500 <?php unset($message);
                ...
                <div class="space-y-6">
                    <div>
                        ...
                        <textarea name="description" id="description" rows="3"
                            ...
                            placeholder="Penjelasan singkat mengenai keunggulan produk..."><?php echo e(old('description', $product->description)); ?></textarea>
                    </div>
                    ...
                    <div>
                        ...
                        <textarea name="features" id="features" rows="5"
                            ...
                            placeholder="• A7075 Aerospace Technology&#10;• T-Floating design&#10;• Tapered tornado-holes"><?php echo e(old('features', $product->features)); ?></textarea>
                    </div>
                </div>
            </div>
        </div>
                ...
                <div class="bg-slate-950 rounded-2xl border border-slate-700 p-3 shadow-2xl flex flex-col justify-center">
                    ...
                    <div class="relative group aspect-video lg:aspect-[16/10] xl:aspect-[16/9] lg:h-[220px] bg-black rounded-xl border border-slate-800 overflow-hidden flex items-center justify-center p-2">
                        ...
                        <div class="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 flex items-center justify-center transition-all backdrop-blur-sm">
                            <label for="image" class="cursor-pointer bg-red-500 hover:bg-black text-white px-4 py-2 rounded-lg font-black text-[9px] uppercase tracking-[0.2em] shadow-2xl transition-all hover:scale-110">Ganti Foto</label>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-slate-900 rounded-2xl border border-slate-700 shadow-2xl flex flex-col overflow-hidden min-h-[220px]">
                    ...
                    <div class="p-3 border-b border-slate-800 space-y-2 bg-slate-950/30">
                        <div class="flex items-center justify-between">
                            ...
                            <span class="text-[7px] text-slate-700 font-black uppercase" x-text="(gallery.length + newGalleryPreviews.length) + ' FOTO'"></span>
                        </div>
                        
                        <div class="flex flex-wrap gap-4 max-h-[220px] overflow-y-auto pr-1 custom-scrollbar">
                            <!-- Existing -->
                            <template x-for="(path, index) in gallery" :key="path">
                                <div class="relative w-28 h-24 rounded-lg overflow-hidden group border border-slate-800 bg-black/40 p-1 hover:border-red-500 transition-all flex-shrink-0 cursor-pointer" @click="mainPreview = '/storage/' + path">
                                    <img :src="'/storage/' + path" class="w-full h-full object-contain">
                                    <button type="button" @click.stop="removeExisting(path)" 
                                        class="absolute top-0 right-0 bg-slate-800/90 text-white p-1.5 rounded-bl-md hover:bg-red-500 transition-all opacity-0 group-hover:opacity-100 shadow-lg">
...
if ($__bag->has($__errorArgs[0])) :
if (isset($message)) { $__messageOriginal = $message; }
$message = $__bag->first($__errorArgs[0]); ?> <p class="text-[8px] text-red-500 text-center font-bold"><?php echo e($message); ?></p> <?php unset($message);
if (isset($__messageOriginal)) { $message = $__messageOriginal; }
endif;
unset($__errorArgs, $__bag); ?>
                    </div>
            ...
            <!-- Hidden inputs to track removals -->
            <template x-for="removed in removedGallery" :key="'rem-'+removed">
                <input type="hidden" name="remove_gallery[]" :value="removed">
            </template>
        </div>
        ...
        <div class="bg-slate-800 rounded-2xl border border-slate-700 p-8 space-y-6" 
             ...
             <!-- ... existing spec builder logic remains same but container upgraded to p-8 ... -->
                    ...
                    <table class="w-full text-left">
                                ...
                                <template x-for="(header, hIdx) in headers" :key="'h-'+hIdx">
                                    <th class="p-4 border-r border-slate-800 last:border-0">
                                        <div class="flex items-center gap-3">
                                            <input type="text" x-model="headers[hIdx]" class="w-full bg-slate-800 border border-slate-700 rounded-lg px-3 py-2 text-[10px] font-black text-white hover:border-red-500 transition-all">
                        ...
                        <tbody>
                            <template x-for="(row, rIdx) in rows" :key="'r-'+rIdx">
                                <tr class="hover:bg-slate-900/40 border-b border-slate-800/50 last:border-0">
                                    <template x-for="(cell, cIdx) in row" :key="'c-'+rIdx+'-'+cIdx">
                                        <td class="p-2 border-r border-slate-800/50 last:border-0 text-slate-400">
                                            <input type="text" x-model="rows[rIdx][cIdx]" class="w-full bg-transparent border-0 focus:ring-0 text-sm py-2">
                                        </td>
                                    </template>
                                    <td class="p-2 w-10">
            ...
            <input type="hidden" name="technical_specs" id="technical_specs_hidden" value="<?php echo e(old('technical_specs', $product->technical_specs)); ?>" :value="jsonValue">
        </div>

        <!-- SECTION 4: KOMPATIBILITAS (Sudah Full Width) -->
        <div class="bg-slate-800 rounded-2xl border border-slate-700 p-8 space-y-8" x-data="{ compatibility: <?php echo e($product->motorcycles->map(fn($m) => ['motorcycle_id'=>$m->id,'diameter'=>$m->pivot->diameter,'color'=>$m->pivot->color,'part_number'=>$m->pivot->part_number])->toJson()); ?> }">
            <div class="flex justify-between items-center">
                <div class="flex items-center space-x-3">
                    <div class="w-1 h-6 bg-red-500 rounded-full"></div>
                ...
                <button type="button" @click="compatibility.push({ motorcycle_id: '', diameter: '', color: '', part_number: '' })" 
                    class="bg-red-500 text-white px-5 py-2.5 rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-red-600 transition-all shadow-lg shadow-red-500/20 flex items-center gap-2">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                <template x-for="(item, index) in compatibility" :key="index">
                    <div class="bg-slate-950 p-6 rounded-2xl border border-slate-800 space-y-4 relative group">
                        <button type="button" @click="compatibility.splice(index, 1)" class="absolute top-4 right-4 text-slate-700 hover:text-red-500 transition-colors">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
                        </button>
                        ...
                        <div>
                            ...
                            <select :name="'compatibility['+index+'][motorcycle_id]'" x-model="item.motorcycle_id" required
                                class="w-full bg-slate-900 border border-slate-700 rounded-lg px-4 py-3 text-xs text-white focus:border-red-500 appearance-none">
                                <option value="">-- Pilih Motor --</option>
                                <?php $__currentLoopData = $motorcycles->groupBy('brand'); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $brand => $models): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <optgroup label="<?php echo e(strtoupper($brand)); ?>">
                                        <?php $__currentLoopData = $models; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $motor): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                            <option value="<?php echo e($motor->id); ?>"><?php echo e($motor->model_name); ?></option>
                                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                                    </optgroup>
                                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                            </select>
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /var/www/resources/views/admin/products/edit.blade.php ENDPATH**/ ?>