...
<div class="w-full" x-data="{ 
    mainPreview: '{{ $product->image_path ? asset('storage/' . $product->image_path) : '' }}',
    ...
    <form action="{{ route('admin.products.update', $product) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                ...
                <div class="space-y-6">
                    <div>
                        ...
                        <textarea name="description" id="description" rows="3"
                            ...
                            placeholder="Penjelasan singkat mengenai keunggulan produk...">{{ old('description', $product->description) }}</textarea>
                    </div>
                    ...
                    <div>
                        ...
                        <textarea name="features" id="features" rows="5"
                            ...
                            placeholder="• A7075 Aerospace Technology&#10;• T-Floating design&#10;• Tapered tornado-holes">{{ old('features', $product->features) }}</textarea>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="bg-slate-800 rounded-2xl border border-slate-700 p-8 space-y-6" 
            ...
            <input type="hidden" name="technical_specs" id="technical_specs_hidden" value="{{ old('technical_specs', $product->technical_specs) }}" :value="jsonValue">
        </div>

        <!-- SECTION 4: KOMPATIBILITAS (Sudah Full Width) -->
        <div class="bg-slate-800 rounded-2xl border border-slate-700 p-8 space-y-8" x-data="{ compatibility: {{ $product->motorcycles->map(fn($m) => ['motorcycle_id'=>$m->id,'diameter'=>$m->pivot->diameter,'color'=>$m->pivot->color,'part_number'=>$m->pivot->part_number])->toJson() }} }">
            <div class="flex justify-between items-center">
                <div class="flex items-center space-x-3">
                    <div class="w-1 h-6 bg-red-500 rounded-full"></div>
