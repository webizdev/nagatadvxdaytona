...
<div class="bg-slate-800 rounded-2xl border border-slate-700 overflow-hidden">
    <div class="p-6 border-b border-slate-700 flex justify-between items-center">
        ...
        <a href="{{ route('admin.products.create') }}" class="px-4 py-2 bg-red-500 hover:bg-red-600 text-white rounded-lg font-bold transition-all flex items-center space-x-2">
                ...
                <tr class="border-b border-slate-700/50 hover:bg-slate-700/20 transition-all">
                    <td class="py-4 px-2">
                        <div class="flex items-center space-x-3">
                            ...
                            <div>
                                <p class="text-slate-100 font-medium">{{ $prod->name }}</p>
                                <p class="text-xs text-slate-500 truncate max-w-[200px]">{{ $prod->slug }}</p>
                            </div>
                        </div>
                    </td>
                    <td class="py-4 px-2 text-slate-400 font-mono">{{ $prod->sku }}</td>
                    <td class="py-4 px-2">
                        <span class="px-2 py-1 rounded-md bg-slate-700 text-slate-300 text-xs">{{ $prod->category->name ?? 'Uncategorized' }}</span>
                    </td>
                    <td class="py-4 px-2 text-right">
                        <div class="flex justify-end space-x-2">
                            <a href="{{ route('admin.products.edit', $prod) }}" class="p-2 text-blue-400 hover:bg-blue-400/10 rounded-lg transition-all">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                            </a>
                            <form action="{{ route('admin.products.destroy', $prod) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus produk ini?')" class="inline">
                                @csrf
                                @method('DELETE')
