...
@section('content')
    ...
    <div class="bg-slate-50 py-16 px-4 sm:px-6 lg:px-8 min-h-screen">
        <div class="max-w-7xl mx-auto">
            ...
            @forelse($dealersByCity as $city => $dealers)
                        ...
                        @foreach($dealers as $dealer)
                            <div class="bg-white rounded-xl p-5 shadow-sm border border-slate-200 hover:shadow-lg hover:border-daytona-orange/50 transition-all duration-300 flex flex-col relative group">
                                <div class="flex items-start justify-between mb-3 gap-2">
                                    ...
                                    <!-- Prominent Map Link -->
                                    <a href="https://maps.google.com/?q={{ urlencode($dealer->nama_toko . ' ' . $dealer->alamat . ' ' . $city) }}" target="_blank" title="Buka di Peta"
                                ...
                                @if($dealer->no_telp)
                                    <div class="pt-3 border-t border-slate-100 mt-auto">
                                        <a href="tel:{{ preg_replace('/[^0-9+]/', '', $dealer->no_telp) }}" class="inline-flex items-center gap-2 text-[12px] font-bold tracking-wide uppercase text-slate-500 hover:text-daytona-orange transition-colors">
